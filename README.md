...
Prints 14 empirical claims with their **measured** runtime values and exits non-zero if any single claim fails: model SHA256 freeze hash, sub-30ms latency SLA, dataset feature rules, 11-layer hybrid risk fusion, and test suite verification. Takes less than 5 seconds.
...
│ (Flutter / React Sandbox) │       │ (CORS + Rate-Limiting)     │       │  (DataSet.csv, F3912 Excluded)    │
└───────────────────────────┘       └────────────────────────────┘       └─────────────────┬─────────────────┘
                                                                                           │
                                  ┌────────────────────────────────────────────────────────┴──────────────────┐
                                  │                                                                           │
...
### 1. 🔒 Cryptographic Model Freeze & Integrity Guarantee
...
### 2. ⚡ High-Throughput Sub-30ms Inference Engine
...
### 3. 🧠 11-Layer Deterministic & Behavioral Security Matrix
...
### 4. 📱 Multi-Platform Frontends & Real-Time Analytics
* **Flutter Mobile App (`fraudguard_flutter`)**: Cross-platform Android/iOS/Web end-user wallet UI with instant visual feedback (Safe Green, Warning Amber, Blocked Red).
...
The active production model `FraudGuard` was trained on `DataSet.csv` containing **9,082 rows and 3,925 raw features**, reduced to the top **100 most predictive features**.
...
> [!NOTE]
> **Dataset Separation Note**: 100% benchmark performance is achieved on the validated test split of `DataSet.csv`. The dataset exhibits high class separability. Production deployment features continuous drift monitoring and human-in-the-loop review.
...
* **Mobile Frontend**: Flutter 3.x (`fraudguard_flutter`), Dart
...
│   ├── model_metadata.json             # Model metadata & benchmark specs
...
Built for the **iQOO Hackathon**. Released under the **Apache 2.0 License**.
