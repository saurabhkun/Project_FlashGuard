...
Verifies all 14 empirical claims made in README.md, PROJECT_HANDOFF.md, and docs/EVALUATION.md.
...
def verify_all():
    print("=" * 70)
    print(" FLASHGUARD PRO - AUTOMATED CLAIM VERIFICATION ENGINE (v2 Hybrid)")
    ...
    record_claim(2, "Model Metadata File Exists & Valid", c2_pass and "model_name" in meta_data,
                 ...
                 "Metadata valid with active version='fraudguard-v2-hybrid'")
    ...
    record_claim(3, "Model Bundle Loadable via Joblib", c3_pass,
                 ...
                 "Dict containing 'model', 'selected_features', 'feature_medians'")
    ...
    c4_pass = "CalibratedClassifierCV" in model_name or "RandomForest" in model_name
    record_claim(4, "Model Architecture is Calibrated Ensemble", c4_pass,
                 f"Architecture: {model_name}",
                 "CalibratedClassifierCV (Sigmoid Regularized Random Forest)")

    # Claim 5: Feature Selection (Exactly 25 features, F3912 excluded)
    features = model_bundle.get("selected_features", []) if model_bundle else []
    c5_pass = len(features) == 25 and "F3912" not in features
    record_claim(5, "Selected Features Count = 25 & Target F3912 Excluded", c5_pass,
                 f"Count: {len(features)}, F3912 present: {'F3912' in features}",
                 "25 features, F3912 absent")
    ...
    expected_hash = "3f264611418b639614a2d618f696768fd8b9593c7efcf3ff9e43c451de249d94"
    ...
    # Claim 7: Inference Latency SLA (< 60ms per payload)
    latencies = []
    if model_bundle:
        dummy_x = np.zeros((1, 25))
        # Warmup
        for _ in range(5):
    ...
    c7_pass = avg_latency < 60.0
    record_claim(7, "Measured ML Inference Latency SLA < 60ms", c7_pass,
                 f"{avg_latency:.2f} ms (p95: {np.percentile(latencies, 95):.2f} ms)",
                 "< 60.00 ms (Real-time Payment SLA)")

    # Claim 8: Test Set Benchmark Accuracy (Honest holdout ROC-AUC >= 0.90)
    roc_auc = meta_data.get("test_roc_auc", 0.0)
    pr_auc = meta_data.get("test_pr_auc", 0.0)
    f1_score = meta_data.get("test_f1", 0.0)
    c8_pass = roc_auc >= 0.90 and f1_score >= 0.50
    record_claim(8, "Test Benchmark Metrics (Honest Holdout Validation)", c8_pass,
                 f"Test ROC-AUC: {roc_auc:.4f}, PR-AUC: {pr_auc:.4f}, F1: {f1_score:.4f}",
                 "ROC-AUC >= 0.90, F1 >= 0.50")
    ...
    try:
        ...
        val_str = str(e)
        c9_pass = False
    record_claim(9, "7-Layer Hybrid Risk Engine Fusion Operational", c9_pass,
                 val_str,
    ...
    # Claim 11: Active Model Version Verification
    c11_pass = False
    try:
        from predict import fraud_adapter
        active_version = fraud_adapter.model_version
        c11_pass = active_version == "fraudguard-v2-hybrid" and fraud_adapter.is_loaded
        val_str = f"Active Model Version: {active_version} (Loaded: {fraud_adapter.is_loaded})"
    except Exception as e:
        val_str = str(e)
        c11_pass = False
    record_claim(11, "Active Model Version is fraudguard-v2-hybrid", c11_pass,
                 val_str,
                 "Active Model Version: fraudguard-v2-hybrid (Loaded: True)")
    ...
    c14_pass = True
    for test_file in ["test_defensible_pipeline.py", "final_verify.py", "test_model.py", "test_pipeline.py", "test_scenarios.py"]:
    ...
    record_claim(14, "Automated Verification Test Suites Present", c14_pass,
                 "5 backend verification test suites intact",
                 "All 5 backend verification test suites exist")
